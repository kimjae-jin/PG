...
app.get('/api/billing', (req, res) => {
    const sql = `
        SELECT
            ...
            (SELECT COUNT(*) FROM billing_history WHERE project_no = b.project_no) as request_count,
            b.note
...
app.get('/api/projects/:id', (req, res) => { const sql = `SELECT p.*, COALESCE((SELECT SUM(deposit_amount) FROM billing_history WHERE project_no = p.project_no), 0) as billed_amount FROM projects p WHERE p.id = ?`; db.get(sql, [req.params.id], (err, row) => { if (err) return res.status(500).json({ error: err.message }); if (!row) return res.status(404).json({ error: '프로젝트를 찾을 수 없습니다.' }); res.json(row); }); });
