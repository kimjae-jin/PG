            ...
            <tbody className="divide-y divide-separator">
              {filteredProjects.map(project => (
                <tr key={project.id} onClick={() => handleRowClick(project.id)} className="hover:bg-tab-hover cursor-pointer divide-x divide-separator">
                  <td className="p-2 text-center align-middle" style={{width: '5%'}}><span className={`px-2 py-1 text-xs font-semibold rounded-full whitespace-nowrap ${project.status === '완료' ? 'bg-blue-200 text-blue-800' : 'bg-green-200 text-green-800'}`}>{project.status}</span></td>
                  <td className="p-2 text-center font-semibold align-middle" style={{width: '8%'}}>{project.project_no}</td>
                  <td className="p-2 align-middle tracking-tighter" style={{width: '15%'}} title={project.project_name}>{project.project_name.length > 10 ? `${project.project_name.substring(0, 10)}...` : project.project_name}</td>
                  <td className="p-2 align-middle tracking-tighter" style={{width: '15%'}} title={project.client}>{project.client && project.client.length > 10 ? `${project.client.substring(0, 10)}...` : project.client}</td>
                  <td className="p-2 text-right font-mono align-middle whitespace-nowrap" style={{width: '9%'}}>{formatCurrency(project.contract_amount)}</td>
                  <td className="p-2 text-right font-mono align-middle whitespace-nowrap" style={{width: '9%'}}>{formatCurrency(project.equity_amount)}</td>
                  <td className="p-2 text-center font-mono align-middle" style={{width: '5%'}}>{project.equity_rate || 0}%</td>
                  <td className="p-2 text-center font-mono align-middle" style={{width: '5%'}}>{project.progress_rate || 0}%</td>
                  <td className="p-2 text-center align-middle" style={{width: '7%'}}>{formatDate(project.contract_date)}</td>
                  <td className="p-2 text-center align-middle" style={{width: '7%'}}>{formatDate(project.start_date)}</td>
                  <td className="p-2 text-center align-middle" style={{width: '7%'}}>{formatDate(project.end_date)}</td>
                  <td className="p-2 text-center align-middle" style={{width: '7%'}}>{project.status === '완료' ? formatDate(project.end_date) : '-'}</td>
                  <td className="p-2 text-center align-middle" style={{width: '6%'}}>{project.manager}</td>
                </tr>
              ))}
            </tbody>
          </table>
