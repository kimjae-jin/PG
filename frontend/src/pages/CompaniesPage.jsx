...
import CompanyCard from '../components/CompanyCard'; // 신규 카드 컴포넌트

// --- 단일 진실 공급원: 컬럼 정의 (기존 정의 기반으로 재구성) ---
const ALL_COLUMNS = {
...
const CompaniesPage = () => {
  ...
  // API 호출 및 데이터 로딩 (기존 코드 기반 안정성 강화)
  useEffect(() => {
              ...
              <table className="w-full text-sm text-left table-fixed">
                ...
                <tbody className="divide-y divide-separator">
                  {filteredCompanies.map(company => (
                    <tr key={company.id} onClick={() => navigate(`/companies/${company.id}`)} className="hover:bg-tab-hover cursor-pointer">
