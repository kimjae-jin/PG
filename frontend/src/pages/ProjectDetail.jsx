...
const TechniciansTab = () => (
    <div className="h-full flex flex-col">
        <div className="flex-shrink-0 flex justify-between items-center mb-4">
            <h4 className="font-bold text-text-color">전체 참여 기술인</h4>
            <button className="bg-accent text-white font-bold py-2 px-4 rounded hover:bg-accent-hover transition-opacity">+ 기술인 추가</button>
        </div>
        <div className="flex-grow overflow-y-auto rounded-lg border border-separator">
            <p className="p-8 text-center text-text-muted">- 기능 구현 예정 -</p>
        </div>
    </div>
);

const NotesTab = ({ projectId }) => { 
    const [notes, setNotes] = useState([]); const [newNote, setNewNote] = useState(''); const [isSaving, setIsSaving] = useState(false); const [isLoading, setIsLoading] = useState(true); 
    useEffect(() => { setIsLoading(true); fetch(`http://localhost:5001/api/projects/${projectId}/notes`).then(res => res.json()).then(data => { setNotes(data); setIsLoading(false); }).catch(err => { console.error('특이사항 로딩 오류:', err); setIsLoading(false); }); }, [projectId]); 
    const handleSaveNote = async () => { if (!newNote.trim()) return; setIsSaving(true); try { const response = await fetch(`http://localhost:5001/api/projects/${projectId}/notes`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ note: newNote }), }); if (!response.ok) throw new Error('저장 실패'); const savedNote = await response.json(); setNotes(prevNotes => [savedNote, ...prevNotes]); setNewNote(''); } catch (error) { console.error('특이사항 저장 오류:', error); } finally { setIsSaving(false); } }; 
    if (isLoading) return <p className="text-text-muted">특이사항 로딩 중...</p>; 
    return ( 
        <div className="space-y-6"> 
            <div>
                <h4 className="font-bold text-text-color mb-2">특이사항 수동 기록</h4>
                <textarea value={newNote} onChange={(e) => setNewNote(e.target.value)} rows="3" className="w-full p-2 bg-input-bg border border-separator rounded-md text-text-color focus:ring-2 focus:ring-accent focus:outline-none" placeholder="기록할 내용을 입력하세요..."></textarea>
                <button onClick={handleSaveNote} disabled={isSaving || !newNote.trim()} className="mt-2 bg-accent text-white font-bold py-2 px-4 rounded hover:bg-accent-hover disabled:opacity-50 disabled:cursor-not-allowed">{isSaving ? '저장 중...' : '기록 저장'}</button>
            </div> 
            <div>
                <h4 className="font-bold text-text-color mb-2">전체 기록</h4>
                <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2">{notes.sort((a, b) => new Date(b.created_at) - new Date(a.created_at)).map(note => (<div key={note.id} className="flex items-start space-x-3 p-3 bg-tab-inactive rounded-md"><div className="flex-shrink-0 text-accent text-xl pt-1">{note.note.startsWith('[자동]') ? '⚙️' : '👤'}</div><div className="flex-grow"><p className="text-text-color break-words">{note.note}</p><div className="text-xs text-text-muted mt-1"><span>{new Date(note.created_at).toLocaleString('ko-KR')}</span></div></div></div>))}</div>
            </div> 
        </div> 
    ); 
};
...
const FinanceTab = ({ projectId }) => { 
    const [billings, setBillings] = useState([]); const [loading, setLoading] = useState(true); 
    useEffect(() => { setLoading(true); fetch(`http://localhost:5001/api/projects/${projectId}/billing`).then(res => res.json()).then(data => { setBillings(data); setLoading(false); }).catch(err => { console.error("Billing data fetch error:", err); setLoading(false); }); }, [projectId]); 
    if (loading) return <p className="text-text-muted">청구/재무 데이터 로딩 중...</p>; 
    return ( 
        <div className="h-full flex flex-col"> 
            <div className="flex-shrink-0 flex justify-between items-center mb-4">
                <h4 className="font-bold text-text-color">청구 및 입금 내역</h4>
                <button className="bg-accent text-white font-bold py-2 px-4 rounded hover:bg-accent-hover transition-opacity">+ 신규 청구 등록</button>
            </div> 
            <div className="flex-grow overflow-y-auto rounded-lg border border-separator">
                <table className="w-full text-left text-sm whitespace-nowrap">
                    <TableHeader headers={['청구구분', '청구일', '청구금액', '입금일', '입금금액', '미수금', '비고']} />
                    <tbody className="divide-y divide-separator">{billings.length === 0 ? (<tr><td colSpan="7" className="text-center p-8 text-text-muted">- 청구/입금 내역이 없습니다 -</td></tr>) : (billings.map((item, index) => { const outstanding = (item.request_amount || 0) - (item.deposit_amount || 0); return (<tr key={index} className="hover:bg-tab-hover"><td className="p-3 font-semibold">{item.request_type}</td><td className="p-3">{item.request_date ? new Date(item.request_date).toLocaleDateString('ko-KR') : '-'}</td><td className="p-3 text-right">{item.request_amount?.toLocaleString() || 0} 원</td><td className="p-3">{item.deposit_date ? new Date(item.deposit_date).toLocaleDateString('ko-KR') : '-'}</td><td className="p-3 text-right text-green-500 font-semibold">{item.deposit_amount?.toLocaleString() || 0} 원</td><td className={`p-3 text-right font-semibold ${outstanding > 0 ? 'text-red-500' : ''}`}>{outstanding.toLocaleString()} 원</td><td className="p-3 text-xs">{item.note}</td></tr>)}))}</tbody>
                </table>
            </div> 
        </div> 
    ); 
...
const ProjectDetail = () => {
    ...
    useEffect(() => { if (!id) { setError('유효하지 않은 프로젝트 ID입니다.'); setIsLoading(false); return; } setIsLoading(true); fetch(`http://localhost:5001/api/projects/${id}`).then(res => { if (!res.ok) throw new Error('프로젝트 정보를 찾을 수 없습니다.'); return res.json(); }).then(data => { setProject(data); setProjectNo(data.project_no); setIsLoading(false); }).catch(err => { setError(err.message); setIsLoading(false); }); return () => setProjectNo(''); }, [id, setProjectNo]);
    ...
    return (
        // [핵심] 페이지 전체를 수직 Flexbox로 만들고, 높이를 꽉 채웁니다.
