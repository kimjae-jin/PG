...
const Sidebar = ({ isCollapsed, onToggle }) => (
    <aside className={`relative bg-sidebar-bg flex-shrink-0 p-2 flex flex-col transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-56'}`}>
        {/* [핵심 수정] "E"를 "ERP"로 변경하고, 홈 링크 기능 추가 */}
        <NavLink 
            to="/" 
            className="text-text-color font-bold text-3xl p-4 text-center h-16 flex items-center justify-center no-underline hover:bg-tab-hover rounded-lg transition-colors duration-200"
            title="홈으로 이동"
        >
            {isCollapsed ? 'E' : 'ERP'}
        </NavLink>
