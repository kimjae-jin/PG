...
// 'status'를 포함한 기본 표시 필드 목록
let visibleFields = ['status', 'projectName', 'clientName', 'startDate', 'endDate', 'totalEquityAmount'];

// 사용자가 선택한 필드 설정을 localStorage에 저장하는 함수
function saveVisibleFields() {
    localStorage.setItem('projectVisibleFields', JSON.stringify(visibleFields));
}

// localStorage에서 저장된 필드 설정을 불러오는 함수
function loadVisibleFields() {
    const savedFields = localStorage.getItem('projectVisibleFields');
    if (savedFields) {
        visibleFields = JSON.parse(savedFields);
    }
}

// 백엔드에서 프로젝트 데이터를 가져와 화면에 렌더링하는 함수
async function fetchProjectsAndRender() {
    try {
        const res = await fetch(`${BACKEND_URL}/api/projects`);
        if (!res.ok) throw new Error('Network error');
        updateProjects(await res.json());
        Components.renderProjectList(projects, visibleFields);
    } catch (err) {
        console.error('Failed to fetch projects:', err);
        const container = document.getElementById('project-list-container');
        if (container) {
            container.innerHTML = `<p class="p-4 text-center text-red-500">데이터 로딩 실패. 백엔드 서버를 확인하세요.</p>`;
        }
    }
}

function setTheme(themeName) {
    document.getElementById('theme-stylesheet').href = `/themes/${themeName}-theme.css`;
...
function updateDarkModeIcon() {
    ...
    if (darkModeToggle) {
        darkModeToggle.innerHTML = htmlEl.classList.contains('dark') ? '☀️' : '🌙';
    }
}

document.addEventListener('DOMContentLoaded', () => {
    loadVisibleFields(); // 페이지 로드 시 저장된 필드 설정 불러오기
    ...
    sidebar.addEventListener('click', (e) => {
        ...
        switch (panelId) {
            case 'project':
                Components.renderProjectContent(mainContent);
                Components.renderFieldSettings(visibleFields);
    ...
    mainContent.addEventListener('input', (e) => {
        ...
        // 스마트 입력: 숫자 천단위 쉼표
        if (name === 'totalAmount' || name === 'totalEquityAmount') {
            const numValue = value.replace(/,/g, '');
            e.target.value = isNaN(numValue) || numValue === '' ? '' : Number(numValue).toLocaleString('ko-KR');
        }
        // 스마트 입력: 'yms' 날짜 변환
        if (type === 'text' && value.toLowerCase() === 'yms') {
    ...
    mainContent.addEventListener('click', async (e) => {
        ...
        if (e.target.id === 'save-project-btn') {
            ...
            const projectData = {
                ...
                projectCategory: form.elements.projectCategory.value,
                pmName: form.elements.pmName.value,
                projectLocation: form.elements.projectLocation.value,
            ...
            const contractData = {
                ...
                totalAmount: form.elements.totalAmount.value.replace(/,/g, ''),
                totalEquityAmount: form.elements.totalEquityAmount.value.replace(/,/g, ''),
            };
            ...
            try {
                const res = await fetch(`${BACKEND_URL}/api/projects`, {
                    ...
                    body: JSON.stringify({ project: projectData, contract: contractData })
                });
                if (!res.ok) {
